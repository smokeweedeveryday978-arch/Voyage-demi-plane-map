...
def export_tiles(im: Image.Image) -> None:
    ...
    manifest = {
        "orientation": "01 is TOP-LEFT; 25 is BOTTOM-RIGHT. Read left-to-right, top-to-bottom.",
        "grid_top_to_bottom": [
            ["01","02","03","04","05"],
            ["06","07","08","09","10"],
            ["11","12","13","14","15"],
        ...
        "tile_encoding": "JPEG quality 100, chroma subsampling disabled",
        "regions": []
    }
    ...
    (ROOT / "voyage_links.json").write_text(
...
def main() -> None:
    src = find_enhanced()
    if not src.exists():
        raise FileNotFoundError(f"Missing approved master source: {src}")
    im = prepare_master(src)
    master_path, q = save_master(im)
    export_tiles(im)
    print(f"master={master_path} size={im.size} master_quality={q}")
    print("01 02 03 04 05")
    print("06 07 08 09 10")
    print("11 12 13 14 15")
    print("16 17 18 19 20")
    print("21 22 23 24 25")
    print("16 = Veiled Fox Sanctuary; 13 = Heartgate Basin; 21 = The Still Frontier; 25 = The Veiled Edge")
